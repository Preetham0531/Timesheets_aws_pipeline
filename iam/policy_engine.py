...
MODULE_TABLE_CONFIG = {
    ...
    "Roles": {
        ...
        "primary_keys": ["rid", "role", "approval_id", "id", "ID"]
    },
    "IAM": {
        "table": "dev.roles_t.ddb-table",
        "owner_field": "createdById",
...
def _get_status(item: Dict[str, Any]) -> str:
    """Enhanced status extraction with debug logging"""
    status_value = (
        item.get("Status") or 
        item.get("status") or 
        item.get("Status") or  # Try both cases
        DEFAULT_STATUS
    ).lower()
    
    logger.debug(f"Status extracted: '{status_value}' from item with keys: {list(item.keys())}")
    return status_value
    ...
    def _rules_from_actions(effect: str, actions_dict: Dict[str, Any],
        ...
        for action, scope in (actions_dict or {}).items():
            ...
            if (selected_creators and action in selected_creators and
                ...
                # Always extract user IDs (dict keys) if dict, never values
                if isinstance(creators, dict):
                    creators = list(creators.keys())
                elif isinstance(creators, list):
                    # If list, filter out any non-userid values (e.g. names)
                    creators = [str(x) for x in creators if x and not isinstance(x, dict)]
                else:
                    creators = [str(creators)]
                # Remove accidental name values (should be only user IDs)
...
def _get_records_by_creators(creator_user_ids: List[str], module: str) -> Set[str]:
    ...
    """
    logger.info(f"🔍 DEBUG: _get_records_by_creators called with {len(creator_user_ids)} creators for module '{module}'")
    logger.info(f"🔍 DEBUG: Creator IDs to query: {creator_user_ids}")
    
    if not creator_user_ids or module not in MODULE_TABLE_CONFIG:
        logger.warning(f"❌ No creator IDs provided or module '{module}' not in config")
        return set()
    ...
    logger.info(f"🔍 DEBUG: Using table '{table_name}', owner_field '{owner_field}', pk_patterns {primary_key_patterns}")
    ...
    try:
        ...
        for creator_id in creator_user_ids:
            ...
            try:
                # Determine GSI name based on owner field
                # For IAM module: uses 'createdById-index' with partition key 'createdById'
                # For other modules: uses 'createdBy-index' with partition key 'createdBy'
                gsi_name = 'createdById-index' if owner_field == 'createdById' else 'createdBy-index'
                use_gsi = True
                
                try:
                    logger.info(f"🔍 DEBUG: Attempting to use GSI '{gsi_name}' for creator {creator_id}")
                    ...
                    for item in response.get('Items', []):
                        for pk_pattern in primary_key_patterns:
                            ...
                            if record_id:
                                ...
                                logger.debug(f"✅ Found record {record_id} created by {creator_id}")
                                break
                    ...
                    while response.get('LastEvaluatedKey'):
                        ...
                        for item in response.get('Items', []):
                            for pk_pattern in primary_key_patterns:
                                ...
                                if record_id:
                                    all_creator_records.add(str(record_id))
                                    logger.debug(f"✅ Found record {record_id} created by {creator_id} (paginated)")
                                    break
                    
                    logger.info(f"✅ GSI query successful for {creator_id}")
                    
                except Exception as gsi_error:
                    # Fallback to scan if GSI query fails
                    logger.warning(f"GSI query failed for {gsi_name}, falling back to scan for {module}: {gsi_error}")
                    use_gsi = False
                    
                    # Fix for reserved keywords like 'role' - use ExpressionAttributeNames
                    expression_attr_names = {}
                    projection_parts = []
                    for i, pk_pattern in enumerate(primary_key_patterns):
                        attr_name = f"#pk{i}"
                        expression_attr_names[attr_name] = pk_pattern
                        projection_parts.append(attr_name)
                    
                    scan_kwargs = {
                        "FilterExpression": Attr(owner_field).eq(creator_id),
                        "ProjectionExpression": ", ".join(projection_parts),
                        "ExpressionAttributeNames": expression_attr_names
                    }
                    
                    logger.info(f"🔍 DEBUG: Scanning with filter {owner_field} = {creator_id}")
                    response = table.scan(**scan_kwargs)
                    items_found = len(response.get('Items', []))
                    logger.info(f"🔍 DEBUG: Scan returned {items_found} items for creator {creator_id}")
                    
                    for item in response.get('Items', []):
                        logger.debug(f"🔍 DEBUG: Processing item: {item}")
                        for pk_pattern in primary_key_patterns:
                            record_id = item.get(pk_pattern)
                            if record_id:
                                all_creator_records.add(str(record_id))
        ...
        logger.info(f"🔍 DEBUG: Total records found: {len(all_creator_records)} from {len(creator_user_ids)} creators in {module}")
        logger.info(f"🔍 DEBUG: Record IDs found: {list(all_creator_records)}")
...
def _gather_rules_for_action(user_id: str, module: str, action: str, assignments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # ========= PROCESS EACH ASSIGNMENT FOR OVERRIDES =========
    for i, assignment in enumerate(assignments):
        try:
            ...
            # Skip role assignments (already processed above)
            if is_role_embedded:
                logger.debug(f"Skipping role assignment {ov_id} (already processed)")
                continue
            
            # Only process overrides
            if not is_override:
                logger.debug(f"Skipping non-override assignment {ov_id}")
                continue
            
            # Check context applicability
            if not _context_applies(assignment, target_ctx):
                logger.info(f"Override {ov_id} context does not apply, skipping")
                skipped_count += 1
                continue
            
            override_count += 1
            logger.debug(f"Processing override {ov_id}")
            
            # Extract policies from override
            policies = assignment.get("Policies", {})
            if isinstance(policies, str):
                try:
                    policies = json.loads(policies)
                except:
                    logger.warning(f"Failed to parse Policies for {ov_id}")
                    continue
            
            # Also check top-level Allow/Deny (direct format)
            if not policies and (assignment.get("Allow") or assignment.get("Deny")):
                policies = {module: {
                    "allow": assignment.get("Allow", {}),
                    "deny": assignment.get("Deny", {}),
                    "SelectedIds": assignment.get("SelectedIds", {}),
                    "DeniedIds": assignment.get("DeniedIds", {}),
                    "SelectedCreators": assignment.get("SelectedCreators", {}),
                    "DeniedCreators": assignment.get("DeniedCreators", {})
                }}
            
            # Build rules from override policies
            override_rules = _build_rules_from_policies(policies, module)
            matching_rules = [r for r in override_rules if _action_matches_pattern(r.get("action", "*"), action)]
            
            # Tag rules as coming from override
            for r in matching_rules:
                r["source"] = "override"
                r["override_id"] = ov_id
                rules.append(r)
            ...
            logger.debug(f"✅ Override {ov_id}: added {len(matching_rules)} rules for action '{action}'")
...
def can_access_record(user_id: str, module: str, action: str, record_id: str) -> bool:
    ...
    try:
        ...
        if record_id_str in denied_ids:
            logger.info(f"❌ Access DENIED for {record_id}: explicitly denied")
            return False

        # Step 2: Check ALLOW patterns
        if scope_result.get("all", False):
            logger.info(f"✅ Access GRANTED for {record_id}: all access (not denied)")
            return True

        # Step 3: Check specific allowed IDs
        allowed_ids = scope_result.get("ids", set())
        is_allowed = record_id_str in allowed_ids
        logger.info(f"{'✅' if is_allowed else '❌'} Access {'GRANTED' if is_allowed else 'DENIED'} for {record_id}: {'in' if is_allowed else 'not in'} allowed list (total allowed: {len(allowed_ids)})")
        return is_allowed
